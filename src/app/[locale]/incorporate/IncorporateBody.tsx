    ...
    <div className="max-w-xl mx-auto px-6 py-12">
      ...
      <div className="space-y-3 mb-6">
        {PKG_INFO.map(({ id, label, desc }) => {
          ...
          return (
            <button key={id} onClick={() => onChange(id)}
              className={`w-full text-left border-2 px-5 py-5 transition-colors ${value === id ? "border-navy-900 bg-navy-50" : "border-gold-200 hover:border-navy-900"}`}>
                ...
                >
                  <div className="grid grid-cols-3 gap-3">
                    {CITIZENSHIP_OPTIONS.map((opt) => (
                      <label
                        key={opt.value}
                        className="flex items-center gap-2 border-2 border-gold-200 px-3 py-2.5 text-sm text-gray-700 cursor-pointer hover:border-navy-900 transition-colors has-[:checked]:border-navy-900 has-[:checked]:bg-navy-50 has-[:checked]:text-navy-900 has-[:checked]:font-medium"
                      >
                        <input
                          type="radio"
                          value={opt.value}
                    ...
                    <label
                      ...
                      className={`block border-2 rounded-lg p-4 cursor-pointer transition-colors ${
                        checked
                          ...
                          : "border-gold-200 bg-white hover:border-navy-900"
                      }`}
                    >
                      <div className="flex items-start gap-3">
                        <input
