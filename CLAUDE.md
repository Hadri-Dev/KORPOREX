...
- **URL pattern**: `localePrefix: "as-needed"` — English (default) is served at the unprefixed root (`/`, `/about`, `/order`); French and Spanish use a prefix (`/fr/about`, `/es/order`). Visiting `/en/*` redirects to the unprefixed equivalent. The `[locale]` directory still drives routing — the param is `en` even when the URL has no prefix.
...
6. **Dynamic data** (e.g. `src/app/[locale]/guides/articles.ts`): structure such that the rendering page reads localized content from messages JSON or a per-locale file. Don't hardcode English-only strings in shared data files that pages render directly.
